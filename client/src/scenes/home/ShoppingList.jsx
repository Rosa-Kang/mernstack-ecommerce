...
const ShoppingList = () => {
    ...
    <Box width="80%" margin="80px auto">
            ...
            <Tabs
                ...
                centered
                TacIndicatorProps={{ sx: { display: isNonMobile ? "block" : "none" } }}
                sx={{
                    m: "25px",
            ...
            <Box
                ...
                columnGap="1.33%"
            >
                {value === "all" && items.map((i) => (
                    <Item item={i} key={`${i.name}-${i.id}`} />
                ))}
                {value === "newArrivals" && newArrivalsItems.map((i) => (
                    <Item item={i} key={`${i.name}-${i.id}`} />
                ))}
                {value === "bestSellers" && bestSellersItems.map((i) => (
                    <Item item={i} key={`${i.name}-${i.id}`} />
                ))}
                {value === "topRated" && topRatedItems.map((i) => (
                    <Item item={i} key={`${i.name}-${i.id}`} />
                ))}
            </Box>
    </Box>
